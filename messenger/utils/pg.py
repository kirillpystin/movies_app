...
CENSORED = "***"
DEFAULT_PG_URL = "postgresql://user:user@0.0.0.0:35432/messenger"
...
async def setup_pg(app: Application, args: Namespace) -> PG:
    ...
    log.info("Подключение к БД: %s", db_info)
    ...
    try:
        ...
        log.info("Отключение от %s", db_info)
        await app["pg"].pool.close()
        log.info("База данных отключена %s", db_info)
